...
if [ -z "$ac_name" ] || [ "$ac_name" = "null" ]; then
	ac_name=$(curl -s -H "X-aws-ec2-metadata-token: $TOKEN" \
		http://169.254.169.254/latest/dynamic/instance-identity/document | jq -r .accountId)
	[ -z "$ac_name" ] || [ "$ac_name" = "null" ] && echo "Error: Account Number is needed" && exit 1
fi
echo "Account number : $ac_name"
generated_token=$(date +%s | sha256sum | base64 | tr -dc _a-z-0-9 | head -c 24)
...
snsprotocol=$(echo "$myurl" | sed -e 's|^\(https\?\)://.*|\1|')
[ -z "$snsprotocol" ] && echo "RG URL must begin with http or https" && exit 1
echo "snsprotocol set to $snsprotocol"
# Strip http:// or https:// prefix for Route53 domain name
r53_domain_name=$(echo "$myurl" | sed -e 's|^https\?://||' -e 's|/$||')
...
tar -C "$RG_HOME" -cvzf "$RG_SRC/config.tar.gz" "config"
tar -tf "$RG_SRC/config.tar.gz"
cp -f "$RG_SRC/config.tar.gz" ./config.tar.gz 2>/dev/null || true
rm -rf "$RG_HOME"
